...
export function Layout({ title, children, hideTitle = false }: { title: string; children: React.ReactNode; hideTitle?: boolean }) { return <ScrollView contentContainerStyle={styles.wrap}><View style={styles.top}>{hideTitle ? <View /> : <Text variant="headlineMedium" style={styles.title}>{title}</Text>}<LanguageSwitch /></View><View style={styles.card}>{children}</View></ScrollView>; }
