...
adminRouter.get('/users', async (req, res) => {
  const users = await prisma.user.findMany({ where: { companyId: req.user!.companyId }, include: { position: true }, orderBy: { createdAt: 'desc' } });
  res.json(users);
});
adminRouter.post('/users', async (req, res) => {
  const d = z.object({
    ...
    paidBreakDefault: z.boolean().default(false),
    profileReviewRequired: z.boolean().default(true),
    passwordChangeRequired: z.boolean().default(true)
  }).parse(req.body);
  const { password, ...userData } = d;
  const user = await prisma.user.create({ data: { ...userData, email: d.email.trim().toLowerCase(), birthDate: d.birthDate ? new Date(d.birthDate) : undefined, passwordHash: await hashPassword(password), companyId: req.user!.companyId } });
...
adminRouter.put('/users/:id', async (req, res) => {
  const d = z.object({
    ...
    address: z.string().optional().nullable(),
    phone: z.string().optional().nullable(),
    profileReviewRequired: z.boolean().optional(),
    passwordChangeRequired: z.boolean().optional(),
    positionId: z.string().optional().nullable(),
    hourlyRate: z.number().optional().nullable(),
    paidBreakDefault: z.boolean().optional(),
    isActive: z.boolean().optional()
  }).parse(req.body);
  const user = await prisma.user.update({ where: { id: String(req.params.id), companyId: req.user!.companyId } as any, data: { ...d, birthDate: d.birthDate ? new Date(d.birthDate) : d.birthDate } as any });
  res.json(user);
});
